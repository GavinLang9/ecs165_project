...
print("Prints base records")
for i in range(0,number_of_records):
    rec = grades_table.get_record(i)
    print(f'rid:{rec.rid}  key:{rec.key}  columns:{rec.columns}')


print("\nThis Select output should display newly merged base records and then latest records of base records that have not been merged")
for i in range(0,number_of_records):
    #rec = grades_table.get_record(i)
